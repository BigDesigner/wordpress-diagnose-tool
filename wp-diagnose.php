 ...
 *
 * v1.1 - Security improvements and minor fixes.
 */
...
<head>
    ...
    <style>
        ...
        .wp-header .actions a:hover {
            background: #e3e3e5;
            border-color: #aaa
        }

        .wp-header .actions a:active {
            background: #d3d3d5;
            border-color: #999;
        ...
        <div class="wp-content">
            ...
            <div class="wp-main">
                ...
                if ($WP_LOADED && isset($_GET['action'])) {
                    ...
                    $a = $_GET['action'];
                    if ($a === 'deactivate_plugin' && !empty($_GET['plugin'])) {
                        deactivate_plugins($_GET['plugin'], true);
                        $MSG = '<div class="notice notice-success">' . t('plugins') . ': ' . t('deactivate') . ' OK</div>';
                    }
                    ...
                    if ($a === 'update_plugin' && !empty($_GET['plugin'])) {
                        $up = new Plugin_Upgrader(new Automatic_Upgrader_Skin());
                        $up->upgrade($_GET['plugin']);
                        $MSG = '<div class="notice notice-success">' . t('plugins') . ': ' . t('update') . ' OK</div>';
                    }
                    if ($a === 'update_all_plugins') {
                        $upd = get_site_transient('update_plugins');
                        $list = array_keys($upd->response ?? []);
                        if ($list) {
                            $up = new Plugin_Upgrader(new Automatic_Upgrader_Skin());
                            foreach ($list as $p) {
                                $up->upgrade($p);
                            }
                            $MSG = '<div class="notice notice-success">' . t('update_all_plugins') . ' OK</div>';
                        } else
                            $MSG = '<div class="notice notice-info">' . t('update_all_plugins') . ': 0</div>';
                    }
                    if ($a === 'update_theme' && !empty($_GET['theme'])) {
                        $up = new Theme_Upgrader(new Automatic_Upgrader_Skin());
                        $up->upgrade($_GET['theme']);
                        $MSG = '<div class="notice notice-success">' . t('themes') . ': ' . t('update') . ' OK</div>';
                    }
                    if ($a === 'update_all_themes') {
                        ...
                        if ($list) {
                            ...
                            $MSG = '<div class="notice notice-success">' . t('update_all_themes') . ' OK</div>';
                        } else
                            $MSG = '<div class="notice notice-info">' . t('update_all_themes') . ': 0</div>';
                    }
                    if ($a === 'update_core') {
                        ...
                        if (!empty($offers[0])) {
                            $res = $up->upgrade($offers[0]);
                            $MSG = is_wp_error($res) ? '<div class="notice notice-error">' . h($res->get_error_message()) . '</div>' : '<div class="notice notice-success">' . t('update_core') . ' OK</div>';
                        } else
                            $MSG = '<div class="notice notice-info">' . t('update_core') . ': 0</div>';
                    }
                    if ($a === 'flush_permalinks') {
                        flush_rewrite_rules(true);
                        $MSG = '<div class="notice notice-success">' . t('flush_permalinks') . ' OK</div>';
                    }
                    if ($a === 'clear_transients') {
                        global $wpdb;
                        $c = $wpdb->query("DELETE FROM {$wpdb->options} WHERE option_name LIKE '\_transient\_%' OR option_name LIKE '\_site\_transient\_%'");
                        $MSG = '<div class="notice notice-success">' . t('clear_transients') . ': ' . intval($c) . '</div>';
                    }
                    ...
                    if ($a === 'delete_maintenance') {
                        $f = ABSPATH . '.maintenance';
                        if (is_file($f))
                            @unlink($f);
                        $MSG = '<div class="notice notice-success">' . t('delete_maintenance') . ' OK</div>';
                    }
                    if ($a === 'force_sitemap_on') {
                        $dir = WP_CONTENT_DIR . '/mu-plugins';
                        if (!is_dir($dir))
                            @mkdir($dir, 0755, true);
                        @file_put_contents($dir . '/force-sitemaps.php', "<?php\n/* Plugin Name: Force Enable WP Sitemaps */\nadd_filter('wp_sitemaps_enabled','__return_true');\n");
                        $MSG = '<div class="notice notice-success">' . t('sitemap_on') . ' OK</div>';
                    }
                    if ($a === 'force_sitemap_off') {
                        $f = WP_CONTENT_DIR . '/mu-plugins/force-sitemaps.php';
                        if (is_file($f))
                            @unlink($f);
                        $MSG = '<div class="notice notice-success">' . t('sitemap_off') . ' OK</div>';
                    }
                }
                ...
                if (!$WP_LOADED && $DB) {
                    if (isset($_GET['db_action'])) {
                        ...
                        if ($a === 'db_deactivate' && !empty($_GET['plugin'])) {
                            ...
                            $arr = @unserialize($ser, ['allowed_classes' => false]);
                            if (!is_array($arr))
                                $arr = [];
                        ...
                        if ($a === 'delete_maintenance') {
                            $f = __DIR__ . '/.maintenance';
                            if (is_file($f))
                                @unlink($f);
                            echo '<div class="notice notice-success">' . t('delete_maintenance') . ' OK</div>';
                        }
                        if ($a === 'force_sitemap_on') {
                        ...
                        if ($a === 'force_sitemap_off') {
                            $f = __DIR__ . '/wp-content/mu-plugins/force-sitemaps.php';
                            if (is_file($f))
                                @unlink($f);
                            echo '<div class="notice notice-success">' . t('sitemap_off') . ' OK</div>';
                        }
                        if ($a === 'clear_error_logs') {
                            $cleared = 0;
                            $iterator = new RecursiveIteratorIterator(new RecursiveDirectoryIterator(ABSPATH, FilesystemIterator::SKIP_DOTS));
                ...
                if (!$WP_LOADED) {
                    if (!empty($_SESSION['db_msg'])) {
                        echo '<div class="notice notice-success">' . h($_SESSION['db_msg']) . '</div>';
                    ...
                    echo '</form>';
                    if ($DB_ERR)
                        echo '<div class="notice notice-error">' . h($DB_ERR) . '</div>';
                }
                    ...
                    if (is_wp_error($api_response) || wp_remote_retrieve_response_code($api_response) !== 200) {
                        ...
                        if (empty($checksums['checksums'])) {
                            ...
                            foreach ($local_files as $file_path => $hash) {
                                ...
                                if (
                                    strpos($file_path, 'wp-content/uploads/') === 0 ||
                ...
                if ($WP_LOADED || $DB) {
                    ...
                    foreach ($rows as $r) {
                        echo '<tr><td>' . h($r[0]) . '</td><td>' . badge($r[1]) . '</td><td>' . h($r[2]) . '</td></tr>';
                    }
                    echo '</table>';
                } else {
                    echo '<div class="notice notice-info">' . t('full_needed') . '</div>';
                ...
                if ($WP_LOADED) {
                    ...
                    $r1 = wp_remote_get(home_url('/wp-sitemap.xml'));
                    $c1 = is_wp_error($r1) ? 0 : wp_remote_retrieve_response_code($r1);
                    $r2 = wp_remote_get(home_url('/sitemap_index.xml'));
                    $c2 = is_wp_error($r2) ? 0 : wp_remote_retrieve_response_code($r2);
                ...
                if ($WP_LOADED) {
                    ...
                    $needUpd = array_keys($updates->response ?? []);
                    ...
                    foreach ($plugins as $file => $data) {
                        ...
                        $acts = [];
                        if ($isActive)
                            $acts[] = '<a class="button" href="?action=deactivate_plugin&plugin=' . rawurlencode($file) . $nonce . '#plugins" onclick="return confirmAct(\'' . t('nonce_confirm') . '\', this.href);">' . t('deactivate') . '</a>';
                        else
                            $acts[] = '<a class="button-primary" href="?action=activate_plugin&plugin=' . rawurlencode($file) . $nonce . '#plugins" onclick="return confirmAct(\'' . t('nonce_confirm') . '\', this.href);">' . t('activate') . '</a>';
                        if ($isUpdateNeeded)
                            $acts[] = '<a class="button-primary" href="?action=update_plugin&plugin=' . rawurlencode($file) . $nonce . '#plugins" onclick="return confirmAct(\'' . t('nonce_confirm') . '\', this.href);">' . t('update') . '</a>';
                    ...
                    if ($DB) {
                        $ser = $DB->get_option('active_plugins');
                        $active = @unserialize($ser, ['allowed_classes' => false]);
                        if (!is_array($active))
                            $active = [];
                ...
                if ($WP_LOADED) {
                    $rest = wp_remote_get(home_url('/wp-json/'));
                    $code = is_wp_error($rest) ? 0 : wp_remote_retrieve_response_code($rest);
                    echo '<hr>';
